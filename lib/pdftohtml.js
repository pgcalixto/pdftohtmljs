  ...
  pdftohtml.prototype.convert = function(preset) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      child.stderr.on('data', (data) => {
        ...
        if (lastLine[0].match(/^internal error: attempt to output/i)) {
          if (!child.killed) {
            child.kill()
          }
          reject(new Error(`${self.options.bin} ran with parameters: ${self.options.additional.join(' ')} terminated using SIGTERM, with following error:\n${error}`));
        }
